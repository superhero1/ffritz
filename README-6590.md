...
FB6490/Puma6 block diagram, Firmware 6.83 (no claim for correctness)
<pre>
```
FB6490/Puma6 block diagram, Firmware 6.8x (no claim for correctness)
+----------------------------+Puma 6+---------------------------------------+ +------------+
|                                                                           | |802.11b+g   |
|    +---+ARM Core (NP CPU)+----+  +---+ATOM Core (APP CPU)+--+             | |PCI:2:0.0   |
|    |                          |  | NAS, USB, WebIF, VOIP,   |             | |AR5002X     |
+----+---+    Box mgmt (PP, CM) |  | WLAN, VPN,               |             | +------+-----+
| Phones?|    EVA(?)            |  | DVB+C Strm +-------------+  +----------+        |
+----+---+                      +  +            |        |ath0+--+PCI:0:1c.0+--------+
|    |                          MBOX            |        +----+  +----------+
|    |                          +  +            |        |ath1+--+PCI:0:1c.1+--------+
|    |                          |  |            |        +----+  +----------+        |
...
|    +----------+   +-----------+  |            +----------+--+  +----------+
|    |wan/      |   |acc0       |  +----------------+      |  |             |
|    |erouter/..|   |169.254.1.2+--+acc0 169.254.1.1|      |  +----+        |
|    +---+----------------------+  +--------------------------+    |        |
|        |                                                 |     +-+--------+
|    +---+--------------+          +-----------------------+-+   |USB2(EHCI)|
|    |         ?        |          |P5                     P0|   +----------+
|    | Packet Processor |          |                         |              |
|    | proxy/NAT/router/|          | Internal L2 Switch  P5/6+---+          |
|    | ...              +----------+P7  P2   P3      P1      |   |          |
|    +---+--------------+          +----+-----+-------+------+  ++---+      |
|        |                              |     |       |         |UDMA|      |
|        |                              |     |     +-+--+      +----+      |
...
+---------------------------------------------------------+
|0x0a0000|0xa0000|config  |mtd3|mtd4|mtd5  |JFFS2 /n^ram  |
+---------------------------------------------------------+
...
FB6490/Puma6 block diagram (up to FW 6.63, no claim for correctness)
<pre>
```
+----------------------------+Puma 6+---------------------------------------+
|                                                                           |
...
|    |  |VLAN 99 | 169.254.1.1| |  | |guest br| | [x.x.x.254] |             |
|    |  |        | VLAN 2     | |  | |VLAN 99 | | 169.254.1.2 |             |
|    |  +----+---+------------+ |  | |        | |VLAN 2       |             |
|    |       |    l2sd0       | |  | +--------+-------+-------+  +----------+
|    |       +-------------+--+ |  | |      eth0      |       +--+SATA Gen2 n.c.
|    |                     |    |  | +----+-----------+       |  +----------+
