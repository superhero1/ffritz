...
"   -f : Don't run service is binary does not exist, stop if it no longer exists.\n"
...
static int service_info (char *service_name, int verbose)
{
	...
	if (WIFEXITED(st.last_exit_code))
	    printf ("exit(%d)", (int)(signed char)WEXITSTATUS(st.last_exit_code));
...
static int kill_service (char *service_name, int sig)
{
	...
	if (errno == ESRCH)
	{
		/* looks like a stale handle */
		purge_service(service_name);
	}
    }
...
int main(int argc, char **argv)
{
   ...
   /* exec ourselves to make sure ffd_service_name appears in 
    ...
    svc_info ("CHECKF=%d", check_file, NULL);
	...
	switch (daemon2 (interval, start_delay, loops, 0, noclose, service_name))
	{
		...
		break;
	    case 3:
		/* client exit due to ENOENT (exe file not found) and -f set */
		purge_service (service_name);
		exit (2);
		break;
	    default:
