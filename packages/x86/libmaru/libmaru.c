...
static int
find_interface_class_index (const struct libusb_config_descriptor *conf,
...
static bool parse_audio_format (const uint8_t * data, size_t size,
				struct maru_stream_desc *desc);

static int perform_pitch_request (maru_context * ctx,
				  unsigned ep, maru_usec timeout);

static maru_error perform_rate_request (maru_context * ctx,
					unsigned ep,
					unsigned rate, maru_usec timeout);

static bool
format_matches (const struct libusb_interface_descriptor *iface,
...
static bool
conf_is_audio_class (const struct libusb_config_descriptor *conf)
{
...
static bool
device_is_audio_class (libusb_device * dev)
{
...
static bool
fill_vid_pid (libusb_device * dev, struct maru_audio_device *audio_dev)
{
...
static bool
enumerate_audio_devices (libusb_device ** list, unsigned devices,
			 struct maru_audio_device **audio_list,
...
maru_error
maru_list_audio_devices (struct maru_audio_device ** audio_list,
			 unsigned *num_devices)
{
    libusb_context *ctx;
...
static bool
poll_list_add (int epfd, int fd, short events)
{
...
static bool
poll_list_remove (int epfd, int fd)
{
    return epoll_ctl (epfd, EPOLL_CTL_DEL, fd, NULL) == 0;
}

static void
poll_list_unblock (int epfd, int fd, short events)
{
    struct epoll_event event = {
	.events =
	    (events & POLLIN ? EPOLLIN : 0) |
...
static void
poll_list_block (int epfd, int fd)
{
...
static inline void
ctx_lock (maru_context * ctx)
{
    pthread_mutex_lock (&ctx->lock);
}

static inline void
ctx_unlock (maru_context * ctx)
{
    pthread_mutex_unlock (&ctx->lock);
}

static void
poll_added_cb (int fd, short events, void *userdata)
{
...
static void
poll_removed_cb (int fd, void *userdata)
{
    maru_context *ctx = userdata;
...
static bool
poll_list_init (maru_context * ctx)
{
    bool ret = true;
...
static void
poll_list_deinit (maru_context * ctx)
{
...
static struct maru_stream_internal *
fd_to_stream (maru_context * ctx, int fd)
{
...
static struct maru_transfer *
find_vacant_transfer (struct maru_transfer **transfers,
		      size_t length, size_t required_buffer)
{
    for (size_t i = 0; i < length; i++)
    {
	if (!transfers[i]->active
	    && transfers[i]->embedded_data_capacity >= required_buffer)
	    return transfers[i];
    }
...
static bool
append_transfer (struct transfer_list *list, struct maru_transfer *trans)
{
    if (list->size >= list->capacity)
    {
	size_t new_capacity = list->capacity * 2 + 1;
	struct maru_transfer **new_trans =
	    realloc (list->transfers, new_capacity * sizeof (trans));
	if (!new_trans)
	    return false;
...
static struct maru_transfer *
create_transfer (struct transfer_list *list, size_t required_buffer)
{
    struct maru_transfer *trans =
	calloc (1, sizeof (*trans) + required_buffer);
    if (!trans)
	return NULL;
...
static void
transfer_stream_cb (struct libusb_transfer *trans)
{
	...
	if (maru_fifo_read_unlock (transfer->stream->fifo, &transfer->region)
	    != LIBMARU_SUCCESS)
    ...
    {
	if (trans->iso_packet_desc[i].length !=
	    trans->iso_packet_desc[i].actual_length)
	    fprintf (stderr,
		     "Actual length differs from sent length! (Actual: %d, Requested: %d)\n",
		     trans->iso_packet_desc[i].actual_length,
	...
	fprintf (stderr,
		 "Stream callback: Failed transfer ... (status: %d)\n",
		 trans->status);
}

static void
transfer_feedback_cb (struct libusb_transfer *trans)
{
    struct maru_transfer *transfer = trans->user_data;
	...
	uint32_t fraction =
	    (trans->buffer[0] << 0) |
	    (trans->buffer[1] << 8) | (trans->buffer[2] << 16);
...
static void
fill_transfer (maru_context * ctx,
	       struct maru_transfer *trans,
	       ...
	       const unsigned *packet_len, unsigned packets)
{
    libusb_fill_iso_transfer (trans->trans,
			      ctx->handle, trans->stream->stream_ep,
			      // If we're contigous in ring buffer, we can just read directly from it.
			      region->second ? trans->embedded_data : region->
			      first, region->first_size + region->second_size,
			      packets, transfer_stream_cb, trans, 1000);
	...
	memcpy (trans->embedded_data + region->first_size, region->second,
		region->second_size);
    }
...
static bool
enqueue_transfer (maru_context * ctx, struct maru_stream_internal *stream,
		  const struct maru_fifo_locked_region *region,
		  const unsigned *packet_len, unsigned packets)
{
    // If our region is split, we have to make a copy to get a contigous transfer.
    size_t required_buffer =
	region->second_size ? region->first_size + region->second_size : 0;
    ...
    struct maru_transfer *transfer =
	find_vacant_transfer (stream->trans.transfers,
...
static bool
enqueue_feedback_transfer (maru_context * ctx,
			   struct maru_stream_internal *stream)
{
    struct maru_transfer *trans =
	calloc (1, sizeof (*trans) + USB_AUDIO_FEEDBACK_SIZE);
    if (!trans)
	return NULL;
...
static size_t
stream_chunk_size (struct maru_stream_internal *stream)
{
    size_t new_fraction =
...
static void
stream_chunk_size_finalize (struct maru_stream_internal *stream)
{
    // Calculate fractional speeds (async isochronous).
    stream->transfer_speed_fraction += stream->transfer_speed & 0xffff;

    // Wrap-around.
    stream->transfer_speed_fraction =
	(UINT32_C (0xffff0000) & stream->transfer_speed) | (stream->
							    transfer_speed_fraction
							    & 0xffff);
}

static void
handle_stream (maru_context * ctx, struct maru_stream_internal *stream)
{
    ...
    debug_print ("handle_stream: %d packets %d\n", __LINE__, packets);
	...
	epoll_ctl (ctx->epfd, EPOLL_CTL_DEL,
		   maru_fifo_read_notify_fd (stream->fifo), NULL);
	eventfd_write (stream->sync_fd, 1);
    }
}

static void
free_transfers_stream (maru_context * ctx,
		       struct maru_stream_internal *stream)
{
...
static void
free_transfers (maru_context * ctx)
{
...
static void
kill_write_notifications (maru_context * ctx)
{
    ctx_lock (ctx);
...
static void
transfer_control_cb (struct libusb_transfer *trans)
{
	...
	debug_print ("LIBUSB_TRANSFER_COMPLETED:\n");
	buf->error = LIBMARU_SUCCESS;
	break;

    case LIBUSB_TRANSFER_TIMED_OUT:
	debug_print ("LIBUSB_TRANSFER_TIMED_OUT::\n");
	...
	debug_print ("LIBUSB_TRANSFER_STALL::\n");
	{
	    ...
	    int ret;
	    if ((ret =
		 libusb_clear_halt (trans->dev_handle, trans->endpoint)) < 0)
	...
	debug_print ("LIBUSB_TRANSFER_ERROR::\n");
	buf->error = LIBMARU_ERROR_IO;
...
static void
handle_request (maru_context * ctx, int fd)
{
    ...
    *buf = req;
    ...
    libusb_fill_control_setup (buf->data.setup,
			       req.request_type,
			       req.request, req.value, req.index, req.size);
    ...
    debug_print ("libusb_submit_transfer\n");
}
...
#define MAX_EVENTS 16
	...
	for (size_t i = 0; i < num_events; i++)
	{
		...
		debug_print ("handle_request\n");
	    }
	    else
	    {
		libusb_event = true;
		debug_print ("libusb_event\n");
	    }
	}

	if (libusb_event)
	{
	    if (libusb_handle_events_timeout (ctx->ctx, &(struct timeval)
					      {
					      0}) < 0)
	    {
...
static bool
add_stream (maru_context * ctx,
	    unsigned interface, unsigned altsetting,
	    unsigned stream_ep, unsigned feedback_ep)
{
    struct maru_stream_internal *new_streams =
	realloc (ctx->streams,
		 (ctx->num_streams + 1) * sizeof (*ctx->streams));
    if (!new_streams)
	return false;
    ...
    {
    .stream_ep = stream_ep,.feedback_ep = feedback_ep,.stream_interface =
	    interface,.stream_altsetting = altsetting,.sync_fd = -1,};
...
static bool
init_stream_nolock (maru_context * ctx,
		    maru_stream stream, const struct maru_stream_desc *desc)
{
    struct maru_stream_internal *str = &ctx->streams[stream];

    const struct libusb_interface_descriptor *iface =
	&ctx->conf->interface[str->stream_interface].altsetting[str->
								stream_altsetting];
    ...
    if (perform_rate_request (ctx, str->stream_ep, desc->sample_rate, 1000000)
	!= LIBMARU_SUCCESS)
	fprintf (stderr, "Rate request (%d) failed or command not supported by endpoint.\n",
	desc->sample_rate);
    ...
    debug_print ("%s:%d\n", __FILE__, __LINE__);
    if (maru_fifo_set_read_trigger (str->fifo, frag_size) < 0)
    {
    ...
    debug_print ("%s:%d\n", __FILE__, __LINE__);
    if (maru_fifo_set_write_trigger (str->fifo, frag_size) < 0)
    {
	maru_fifo_free (str->fifo);
	str->fifo = NULL;
	return false;
    ...
    debug_print ("%s:%d\n", __FILE__, __LINE__);
    poll_list_add (ctx->epfd, maru_fifo_read_notify_fd (str->fifo), POLLIN);
...
static void
deinit_stream_nolock (maru_context * ctx, maru_stream stream)
{
    struct maru_stream_internal *str = &ctx->streams[stream];
...
static void
deinit_stream (maru_context * ctx, maru_stream stream)
{
    ctx_lock (ctx);
...
static bool
find_interface_endpoints (maru_context * ctx,
			  unsigned interface,
			  unsigned altsetting,
			  const struct libusb_interface_descriptor *iface)
{
    ...
    {
	const struct libusb_endpoint_descriptor *endp = &iface->endpoint[i];
	    ...
	    endp->bmAttributes & USB_ENDPOINT_ISOCHRONOUS)
	{
	    ...
	    return add_stream (ctx,
			       ...
			       endp->bEndpointAddress, endp->bSynchAddress);
	}
    }
...
static bool
enumerate_stream_interfaces (maru_context * ctx,
			     const struct maru_stream_desc *desc)
{
    for (unsigned i = 0; i < ctx->conf->bNumInterfaces; i++)
    {
	unsigned altsettings = ctx->conf->interface[i].num_altsetting;
	...
	for (unsigned j = 0; j < altsettings; j++)
	{
		...
		iface->bNumEndpoints >= 1 && format_matches (iface, desc))
	    {
...
static bool
enumerate_streams (maru_context * ctx, const struct maru_stream_desc *desc)
{
	...
	if (libusb_claim_interface (ctx->handle, ctrl_index) < 0)
	...
	if (libusb_kernel_driver_active (ctx->handle, iface)
	    && libusb_detach_kernel_driver (ctx->handle, iface) < 0)
	    return false;

	if (libusb_claim_interface (ctx->handle, iface) < 0)
	    return false;

	if (libusb_set_interface_alt_setting (ctx->handle, iface, altsetting)
	    < 0)
	    return false;
    }
...
find_volume_feature_unit (const uint8_t * ctrl_extra,
			  size_t ctrl_extra_length,
			  const uint8_t * iface_extra,
			  size_t iface_extra_length)
{
    int term = -1;
	...
	if (feature->bDescriptorSubtype == UAC_FEATURE_UNIT &&
	    feature->bSourceID == term && feature->bControlSize)
	    return feature;
    }
...
find_master_volume_feature_unit (const uint8_t * extra, size_t extra_length)
{
	...
	if (feature->bDescriptorSubtype == UAC_FEATURE_UNIT &&
	    feature->bUnitID == id && feature->bControlSize)
	    return feature;
    }
...
static void
parse_feature_unit (struct volume_control *vol,
		    const struct usb_uac_feature_unit_descriptor *feature)
{
    ...
    for (unsigned i = 0; i < control_len && vol->chans < 2;
	 i += feature->bControlSize)
    {
...
static bool
enumerate_master_controls (maru_context * ctx)
{
    const struct libusb_interface_descriptor *desc =
...
static void
enumerate_stream_controls (maru_context * ctx, maru_stream stream)
{
    const struct libusb_interface_descriptor *iface =
	&ctx->conf->interface[ctx->streams[stream].stream_interface].
	altsetting[ctx->streams[stream].stream_altsetting];

    const struct libusb_interface_descriptor *ctrl =
	&ctx->conf->interface[ctx->control_interface].altsetting[0];

    struct usb_uac_feature_unit_descriptor *feature =
	find_volume_feature_unit (ctrl->extra, ctrl->extra_length,
				  iface->extra, iface->extra_length);
...
static bool
enumerate_controls (maru_context * ctx)
{
...
maru_error
maru_create_context_from_vid_pid (maru_context ** ctx,
				  ...
				  const struct maru_stream_desc * desc)
{
    ...
    if (context->quit_fd < 0 || context->epfd < 0)
	goto error;
    ...
    context->handle =
	libusb_open_device_with_vid_pid (context->ctx, vid, pid);
    if (!context->handle)
	goto error;

    if (libusb_get_active_config_descriptor
	(libusb_get_device (context->handle), &context->conf) < 0)
    {
	context->conf = NULL;
	goto error;
    }
...
void
maru_destroy_context (maru_context * ctx)
{
    if (!ctx)
	return;
	...
	for (unsigned i = 0; i < ctx->num_streams; i++)
	{
	    libusb_release_interface (ctx->handle,
				      ctx->streams[i].stream_interface);
	    libusb_attach_kernel_driver (ctx->handle,
					 ctx->streams[i].stream_interface);
	}
...
int
maru_get_num_streams (maru_context * ctx)
{
    return ctx->num_streams;
}

static bool
parse_audio_format (const uint8_t * data, size_t length,
		    struct maru_stream_desc *desc)
{
	...
	if (header->bSamFreqType == 0)	// Continous
	{
		...
		(header->tSamFreq[0] << 0) |
		(header->tSamFreq[1] << 8) | (header->tSamFreq[2] << 16);
...
static bool
fill_audio_format (maru_context * ctx, maru_stream stream,
		   struct maru_stream_desc *desc)
{
...
maru_error
maru_get_stream_desc (maru_context * ctx,
...
int
maru_is_stream_available (maru_context * ctx, maru_stream stream)
{
    ctx_lock (ctx);
...
int
maru_find_available_stream (maru_context * ctx)
{
    for (unsigned i = 0; i < ctx->num_streams; i++)
...
void
maru_stream_set_write_notification (maru_context * ctx,
				    maru_stream stream,
				    maru_notification_cb callback,
				    void *userdata)
{
    if (stream >= ctx->num_streams)
	return;
...
void
maru_stream_set_error_notification (maru_context * ctx,
				    maru_stream stream,
				    maru_notification_cb callback,
				    void *userdata)
{
...
maru_error
maru_stream_open (maru_context * ctx,
		  maru_stream stream, const struct maru_stream_desc * desc)
{
...
static void
init_timer (struct maru_stream_internal *str)
{
...
size_t
maru_stream_write (maru_context * ctx, maru_stream stream,
...
size_t
maru_stream_write_avail (maru_context * ctx, maru_stream stream)
{
    if (stream >= ctx->num_streams)
	return 0;
...
static maru_error
perform_request (maru_context * ctx,
		 uint8_t request_type, uint8_t request, uint16_t value,
		 uint16_t index, void *data, size_t size, maru_usec timeout)
{
    struct maru_control_request req = {
...
static int
perform_pitch_request (maru_context * ctx, unsigned ep, maru_usec timeout)
{
    return libusb_control_transfer (ctx->handle,
				    LIBUSB_REQUEST_TYPE_CLASS |
				    LIBUSB_RECIPIENT_ENDPOINT,
				    USB_REQUEST_UAC_SET_CUR,
				    UAS_PITCH_CONTROL << 8, ep, (uint8_t[])
				    {
				    1}
				    , sizeof (uint8_t),
...
static maru_error
perform_volume_request (maru_context * ctx,
			...
			maru_volume * vol, uint8_t request, maru_usec timeout)
{
	...
	maru_error err = perform_request (ctx,
					  ...
					  (USB_UAC_VOLUME_SELECTOR << 8) |
					  ctrl->channels[i],
					  (ctrl->feature_unit << 8) | ctx->
					  control_interface,
...
const struct volume_control *
stream_to_volume_control (maru_context * ctx, maru_stream stream)
{
...
maru_error
maru_stream_get_volume (maru_context * ctx,
			maru_stream stream,
			maru_volume * current, maru_volume * min,
			maru_volume * max, maru_usec timeout)
{
    const struct volume_control *ctrl =
...
maru_error
maru_stream_set_volume (maru_context * ctx,
			maru_stream stream,
			maru_volume volume, maru_usec timeout)
{
    const struct volume_control *ctrl =
	stream_to_volume_control (ctx, stream);
    if (!ctrl)
...
static maru_usec
timed_latency (struct maru_stream_internal *stream)
{
    maru_usec since_start = current_time () - stream->timer.start_time;
    maru_usec buffered_time =
	(stream->timer.write_cnt * 1000000) / stream->bps;
...
maru_stream_current_latency (maru_context * ctx, maru_stream stream)
{
    ...
    maru_usec buffer_latency =
	(maru_fifo_buffered_size (str->fifo) * INT64_C (1000000)) / str->bps;
...
const char *
maru_error_string (maru_error error)
{
    switch (error)
    {
