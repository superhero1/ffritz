...
static inline void
fifo_lock (maru_fifo * fifo)
{
    pthread_mutex_lock (&fifo->lock);
}

static inline void
fifo_unlock (maru_fifo * fifo)
{
    pthread_mutex_unlock (&fifo->lock);
}

void
maru_fifo_free (maru_fifo * fifo)
{
    if (!fifo)
	return;
...
int
maru_fifo_write_notify_fd (maru_fifo * fifo)
{
    return fifo->write_fd;
}

int
maru_fifo_read_notify_fd (maru_fifo * fifo)
{
    return fifo->read_fd;
}

static inline size_t
maru_fifo_read_avail_nolock (maru_fifo * fifo)
{
    return (fifo->write_lock_begin + fifo->buffer_size -
	    fifo->read_lock_end) & fifo->buffer_mask;
}

static inline size_t
maru_fifo_write_avail_nolock (maru_fifo * fifo)
{
    return (fifo->read_lock_begin + fifo->buffer_size - fifo->write_lock_end -
	    1) & fifo->buffer_mask;
}

size_t
maru_fifo_buffered_size (maru_fifo * fifo)
{
    return fifo->buffer_size - maru_fifo_write_avail (fifo) - 1;
}

size_t
maru_fifo_read_avail (maru_fifo * fifo)
{
    fifo_lock (fifo);
...
size_t
maru_fifo_write_avail (maru_fifo * fifo)
{
    fifo_lock (fifo);
...
maru_error
maru_fifo_write_lock (maru_fifo * fifo,
		      size_t size, struct maru_fifo_locked_region * region)
{
	...
	fifo->write_lock_end =
	    (fifo->write_lock_end + region->first_size) & fifo->buffer_mask;
...
maru_error
maru_fifo_write_unlock (maru_fifo * fifo,
			const struct maru_fifo_locked_region * region)
{
    ...
    size_t new_begin =
	(fifo->write_lock_begin + region->first_size) & fifo->buffer_mask;
...
maru_error
maru_fifo_read_lock (maru_fifo * fifo,
		     size_t size, struct maru_fifo_locked_region * region)
{
    fifo_lock (fifo);
	...
	fifo->read_lock_end =
	    (fifo->read_lock_end + region->first_size) & fifo->buffer_mask;
...
maru_error
maru_fifo_read_unlock (maru_fifo * fifo,
		       const struct maru_fifo_locked_region * region)
{
    maru_error ret = LIBMARU_SUCCESS;
    fifo_lock (fifo);
    ...
    size_t new_begin =
	(fifo->read_lock_begin + region->first_size) & fifo->buffer_mask;
...
ssize_t
maru_fifo_write (maru_fifo * fifo, const void *data_, size_t size)
{
    ...
    debug_print (">> fifo write %d\n", size);

    return size;
}

ssize_t
maru_fifo_read (maru_fifo * fifo, void *data_, size_t size)
{
    uint8_t *data = data_;
    ...
    debug_print ("<< fifo read %d\n", size);

    return size;
}

size_t
maru_fifo_blocking_write (maru_fifo * fifo, const void *data_, size_t size)
{
    const uint8_t *data = data_;
    size_t written = 0;
...
size_t
maru_fifo_blocking_read (maru_fifo * fifo, void *data_, size_t size)
{
    uint8_t *data = data_;
    size_t has_read = 0;
...
static inline void
maru_fifo_read_notify_ack_nolock (maru_fifo * fifo)
{
...
static inline void
maru_fifo_write_notify_ack_nolock (maru_fifo * fifo)
{
...
maru_error
maru_fifo_read_notify_ack (maru_fifo * fifo)
{
    fifo_lock (fifo);
...
maru_error
maru_fifo_write_notify_ack (maru_fifo * fifo)
{
    fifo_lock (fifo);
    maru_error ret = fifo->dead ? LIBMARU_ERROR_DEAD : LIBMARU_SUCCESS;
    if (!fifo->dead)
...
void
maru_fifo_kill_notification (maru_fifo * fifo)
{
    fifo_lock (fifo);
    fifo->dead = true;
    eventfd_write (fifo->write_fd, 1);
...
maru_error
maru_fifo_set_write_trigger (maru_fifo * fifo, size_t size)
{
    maru_error ret = LIBMARU_SUCCESS;
    fifo_lock (fifo);
...
maru_error
maru_fifo_set_read_trigger (maru_fifo * fifo, size_t size)
{
    maru_error ret = LIBMARU_SUCCESS;
    fifo_lock (fifo);
