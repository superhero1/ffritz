...
parseList() {
  ...
  #  XXX ffritz: "/.$/a\\" adds unwanted newlines causing tons of sql insert warnings
  sed -e "/[^a-zA-Z0-9.\_-]/d;s/$/,${adlistID}/" "${src}" >> "${target}"
...
gravity_DownloadBlocklistFromUrl() {
  ...
  httpCode=$(curl --insecure -s -L ${cmd_ext} ${heisenbergCompensator} -w "%{http_code}" -A "${agent}" "${url}" -o "${patternBuffer}" 2> /dev/null)
